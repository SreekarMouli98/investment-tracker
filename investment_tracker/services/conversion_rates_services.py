...
from investment_tracker.accessors.conversion_rates_accessors import (
    ConversionRatesAccessor,
)
...
class ConversionRatesService:
    def get_conversion_rate(
        ...
        new_conversion_rates = (
            [] if new_conversion_rates is None else new_conversion_rates
        )
        ...
        includes_crypto = (
            from_asset.asset_class.id == crypto_asset_class["id"]
            or to_asset.asset_class.id == crypto_asset_class["id"]
        )
        rate = None
        conv_rate = ConversionRatesAccessor().get_conversion_rate(
            between=[from_asset, to_asset], date=date
        )
        if conv_rate:
            ...
            if from_asset.asset_class.id == crypto_asset_class["id"]:
                ...
                is_inverse = True
            currency_asset_class = AssetClassesService().get_asset_class_by_name(
                "Currency"
            )
            if final_asset.asset_class.id != currency_asset_class["id"]:
                base_asset = get_base_asset()
                crypto_to_base = self.get_conversion_rate(
                    crypto_asset, base_asset, date=date
                )
                base_to_final = self.get_conversion_rate(
                    base_asset, final_asset, date=date
                )
            ...
            conv_rate = ConversionRatesModel(
                from_asset=from_asset, to_asset=to_asset, conv_rate=rate, date=date
            )
            if skip_persist:
                new_conversion_rates.append(conv_rate)
            else:
    ...
    def get_conversion_rate_cached(
